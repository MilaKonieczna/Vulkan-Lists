...
class HelloTriangleApplication(object):
    ...
    def __init__(self):
        ...
        self.__vertices = np.array([
            # pos    color     texCoord
            0, .5, 0, 0, 0, 0.5, 1.0,
            .3535, .3535, 0, 0, 0, 0.8535, 0.8535,
            .5, 0, 0, 0, 0, 1.0, 0.5,
            .3535, -.3535, 0, 0, 0, 0.8535, 0.1465,
            0, -.5, 0, 0, 0, 0.5, 0.0,
            -.3535, -.3535, 0, 0, 0, 0.1465, 0.1465,
            -.5, 0, 0, 0, 0, 0.0, 0.5,
            -.3535, .3535, 0, 0, 0, 0.1465, 0.8535,
    ...
    def __updateProjectionMatrix(self):
        ...
        half_tan_fov = (np.tan( np.radians(45)/2 ))
        far = 10
        near = 0.1
    ...
    def __updateUniformBuffer(self):
        ...
        self.__ubo.model = glm.rotate(np.identity(4, np.float32), t, glm.vec3(0,0,1))
        self.__ubo.view = glm.lookAt(np.array([2, -2, 2], np.float32),
                                     np.array([0, 0, 0], np.float32),
                                     np.array([0, 0, 1], np.float32))
        #self.__ubo.proj = glm.perspective(glm.radians(45), float(self.__swapChainExtent.width) / float(self.__swapChainExtent.height), 0.1, 10.0)
        #self.__ubo.proj *= -1

        self.__updateProjectionMatrix()
        #print ('--')
        #print (self.__ubo.proj)

        self.__ubo.model = np.array(self.__ubo.model)
        self.__ubo.view = np.array(self.__ubo.view)
        #self.__ubo.proj = np.array(self.__ubo.proj)
